...
def monitor_asr_train_progress(tensors: list, labels: list, eval_metric='WER', tb_logger=None):
    ...
    wer = word_error_rate(hypotheses, references, use_cer=False)
    cer = word_error_rate(hypotheses, references, use_cer=True)
    if tb_logger is not None:
        tb_logger.add_scalar(tag, wer)
    logging.info(f'Loss: {tensors[0]}')
    logging.info(f'Train WER: {wer * 100 : 5.2f}% | CER: {cer * 100 : 5.2f}%')
...
def __gather_transcripts(transcript_list: list, transcript_len_list: list, labels: list) -> list:
    ...
    #print('(transcript_list, transcript_len_list):',(transcript_list, transcript_len_list))
...
def process_evaluation_epoch(global_vars: dict, eval_metric='WER', tag=None):
    ...
    if tag is None:
        ...
        logging.info(f"==========>>>>>>Evaluation Valid WER: " f"{wer * 100 : 5.2f}%")
        logging.info(f"==========>>>>>>Evaluation Valid CER: " f"{cer * 100 : 5.2f}%")
        return {"Evaluation_Loss": eloss, "Evaluation_Valid_WER": wer, "Evaluation_Valid_CER": cer}
    else:
        ...
        return {
            ...
            f"Evaluation_Valid_WER": wer,
            f"Evaluation_Valid_CER": cer,
        }
